...
use hathor_ct_crypto_core::types::COMMITMENT_SIZE;
...
fn parse_generator(bytes: &[u8]) -> napi::Result<Generator> {
    ...
    hathor_ct_crypto_core::generators::deserialize_generator(bytes).map_err(to_napi_err)
}
...
pub fn derive_asset_tag(token_uid: Buffer) -> napi::Result<Buffer> {
    ...
    let tag = hathor_ct_crypto_core::generators::derive_asset_tag(&uid).map_err(to_napi_err)?;
...
pub fn derive_tag(token_uid: Buffer) -> napi::Result<Buffer> {
    ...
    let uid: [u8; 32] = token_uid
        ...
        .try_into()
        .map_err(|_| napi::Error::from_reason("token_uid must be exactly 32 bytes"))?;
    let tag = hathor_ct_crypto_core::generators::derive_tag(&uid).map_err(to_napi_err)?;
...
pub fn create_asset_commitment(tag_bytes: Buffer, r_asset: Buffer) -> napi::Result<Buffer> {
    ...
    let commitment =
        hathor_ct_crypto_core::generators::create_asset_commitment(&tag, &tweak).map_err(to_napi_err)?;
...
pub fn create_commitment(
    ...
    let c = hathor_ct_crypto_core::pedersen::create_commitment(amount, &bf, &gen).map_err(to_napi_err)?;
...
pub fn create_trivial_commitment(amount: BigInt, generator: Buffer) -> napi::Result<Buffer> {
    let amount = bigint_to_u64(&amount)?;
    let gen = parse_generator(generator.as_ref())?;
    let c = hathor_ct_crypto_core::pedersen::create_trivial_commitment(amount, &gen).map_err(to_napi_err)?;
    Ok(Buffer::from(c.serialize().to_vec()))
}
...
pub fn verify_commitments_sum(positive: Vec<Buffer>, negative: Vec<Buffer>) -> napi::Result<bool> {
    let pos: Vec<_> = positive
        .iter()
        .map(|b| hathor_ct_crypto_core::pedersen::deserialize_commitment(b.as_ref()).map_err(to_napi_err))
...
pub fn create_range_proof(
    ...
    let amount = bigint_to_u64(&amount)?;
    let bf = parse_tweak(blinding.as_ref())?;
    let comm = hathor_ct_crypto_core::pedersen::deserialize_commitment(commitment.as_ref()).map_err(to_napi_err)?;
    let gen = parse_generator(generator.as_ref())?;
    let nonce_key = nonce
        .as_ref()
...
pub fn verify_range_proof(
    ...
    let p = hathor_ct_crypto_core::rangeproof::deserialize_range_proof(proof.as_ref()).map_err(to_napi_err)?;
    let c = hathor_ct_crypto_core::pedersen::deserialize_commitment(commitment.as_ref()).map_err(to_napi_err)?;
    let gen = parse_generator(generator.as_ref())?;
...
pub fn rewind_range_proof(
    ...
    let (value, blinding, message) =
        hathor_ct_crypto_core::rangeproof::rewind_range_proof(&p, &c, &sk, &gen).map_err(to_napi_err)?;
    Ok(RewindResult {
...
pub fn validate_commitment(data: Buffer) -> bool {
    ...
    hathor_ct_crypto_core::pedersen::deserialize_commitment(data.as_ref()).is_ok()
}
...
pub fn validate_generator(data: Buffer) -> bool {
    ...
    hathor_ct_crypto_core::generators::deserialize_generator(data.as_ref()).is_ok()
}
...
pub fn create_surjection_proof(
    ...
    let proof =
        hathor_ct_crypto_core::surjection::create_surjection_proof(&ct, &cbf, &domain_vec).map_err(to_napi_err)?;
    Ok(Buffer::from(proof.serialize().to_vec()))
}
...
pub fn verify_surjection_proof(
    ...
    let p = hathor_ct_crypto_core::surjection::deserialize_surjection_proof(proof.as_ref()).map_err(to_napi_err)?;
    let codomain_gen = parse_generator(codomain.as_ref())?;
    let domain_gens: Vec<Generator> = domain
        .iter()
        .map(|b| parse_generator(b.as_ref()))
        .collect::<napi::Result<Vec<_>>>()?;
    match hathor_ct_crypto_core::surjection::verify_surjection_proof(&p, &codomain_gen, &domain_gens) {
        Ok(()) => Ok(true),
        Err(_) => Ok(false),
    }
}
...
pub fn verify_balance(
    ...
    for entry in &transparent_inputs {
        ...
        inputs.push(hathor_ct_crypto_core::balance::BalanceEntry::Transparent {
            amount: bigint_to_u64(&entry.amount)?,
            token_uid: uid,
        });
    }
    for cb in &shielded_inputs {
        let c = hathor_ct_crypto_core::pedersen::deserialize_commitment(cb.as_ref()).map_err(to_napi_err)?;
        inputs.push(hathor_ct_crypto_core::balance::BalanceEntry::Shielded {
            value_commitment: c,
        });
    }
    ...
    for entry in &transparent_outputs {
        let uid: [u8; 32] = entry
            ...
            .try_into()
            .map_err(|_| napi::Error::from_reason("token_uid must be 32 bytes"))?;
        outputs.push(hathor_ct_crypto_core::balance::BalanceEntry::Transparent {
            amount: bigint_to_u64(&entry.amount)?,
            token_uid: uid,
        });
    }
    for cb in &shielded_outputs {
        let c = hathor_ct_crypto_core::pedersen::deserialize_commitment(cb.as_ref()).map_err(to_napi_err)?;
        outputs.push(hathor_ct_crypto_core::balance::BalanceEntry::Shielded {
            value_commitment: c,
        });
    }
...
pub fn compute_balancing_blinding_factor(
    ...
    let result =
        hathor_ct_crypto_core::balance::compute_balancing_blinding_factor(value, &gbf, &in_entries, &out_entries)
            .map_err(to_napi_err)?;
...
pub fn derive_rewind_nonce(shared_secret: Buffer) -> napi::Result<Buffer> {
    ...
    let nonce = hathor_ct_crypto_core::ecdh::derive_rewind_nonce(&secret);
...
pub fn create_shielded_output_with_both_blindings(
    ...
    let result = hathor_ct_crypto_core::ecdh::create_full_shielded_output(
        value,
...
pub fn create_amount_shielded_output(
    ...
    let result =
        hathor_ct_crypto_core::ecdh::create_amount_shielded_output(value, recipient_pubkey.as_ref(), &tuid, &vbf)
            .map_err(to_napi_err)?;
